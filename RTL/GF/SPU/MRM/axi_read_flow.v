module axi_read_flow #(
    ...
    // Flow data was produced by the UPPER layer (current_layer + 1).
    always @(*) begin
        case (current_layer)
            3'd0:    total_beats_wire = ((IMG_WIDTH >> 1) * (IMG_HEIGHT >> 1) * BYTES_PER_FLOW) / BYTES_PER_BEAT;
            3'd1:    total_beats_wire = ((IMG_WIDTH >> 2) * (IMG_HEIGHT >> 2) * BYTES_PER_FLOW) / BYTES_PER_BEAT;
            3'd2:    total_beats_wire = ((IMG_WIDTH >> 3) * (IMG_HEIGHT >> 3) * BYTES_PER_FLOW) / BYTES_PER_BEAT;
            3'd3:    total_beats_wire = ((IMG_WIDTH >> 4) * (IMG_HEIGHT >> 4) * BYTES_PER_FLOW) / BYTES_PER_BEAT;
