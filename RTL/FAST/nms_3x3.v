module nms_3x3 #(
    ...
    fast_line_buffer #(.DATA_WIDTH(SW), .LINE_WIDTH(LINE_WIDTH)) u_lb_prev1 (
        .clk(clk), .rst_n(rst_n), .din(score_in), 
        .din_valid(new_pixel), .dout(lb_out_prev1), .dout_valid(lb_vld_prev1)
    );

    fast_line_buffer #(.DATA_WIDTH(SW), .LINE_WIDTH(LINE_WIDTH)) u_lb_prev2 (
    ...
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) begin
            ...
            corner_valid <= is_local_max && lb_vld_prev2 && (row_d2 >= 4);
            corner_score <= center_middle;
            
            corner_col   <= (col_d2 >= 1) ? col_d2 - 1'b1 : {COL_W{1'b0}};
            corner_row   <= (row_d2 >= 4) ? row_d2 - 4 : {ROW_W{1'b0}};
      
            frame_done   <= (col_d2 == LINE_WIDTH - 1) && (row_d2 == IMG_HEIGHT - 1);
        end else begin
            frame_done   <= 1'b0; 
        end
    end
