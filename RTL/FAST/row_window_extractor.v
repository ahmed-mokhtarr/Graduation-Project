module row_window_extractor #(
    ...
    fast_line_buffer #(.DATA_WIDTH(D), .LINE_WIDTH(LINE_WIDTH)) u_lb0 (.clk(clk), .rst_n(rst_n), .din(din),      .din_valid(din_valid), .dout(lb_out_0),.dout_valid(lb_vld_0));
    fast_line_buffer #(.DATA_WIDTH(D), .LINE_WIDTH(LINE_WIDTH)) u_lb1 (.clk(clk), .rst_n(rst_n), .din(lb_out_0), .din_valid(lb_vld_0),  .dout(lb_out_1),.dout_valid(lb_vld_1));
    fast_line_buffer #(.DATA_WIDTH(D), .LINE_WIDTH(LINE_WIDTH)) u_lb2 (.clk(clk), .rst_n(rst_n), .din(lb_out_1), .din_valid(lb_vld_1),  .dout(lb_out_2),.dout_valid(lb_vld_2));
    fast_line_buffer #(.DATA_WIDTH(D), .LINE_WIDTH(LINE_WIDTH)) u_lb3 (.clk(clk), .rst_n(rst_n), .din(lb_out_2), .din_valid(lb_vld_2),  .dout(lb_out_3),.dout_valid(lb_vld_3));
    fast_line_buffer #(.DATA_WIDTH(D), .LINE_WIDTH(LINE_WIDTH)) u_lb4 (.clk(clk), .rst_n(rst_n), .din(lb_out_3), .din_valid(lb_vld_3),  .dout(lb_out_4),.dout_valid(lb_vld_4));
    fast_line_buffer #(.DATA_WIDTH(D), .LINE_WIDTH(LINE_WIDTH)) u_lb5 (.clk(clk), .rst_n(rst_n), .din(lb_out_4), .din_valid(lb_vld_4),  .dout(lb_out_5),.dout_valid(lb_vld_5));
    ...
    wire [ROW_W-1:0] center_row_val = row_d[11];

    wire [COL_W-1:0] true_col_val = col_d[10];
    wire [ROW_W-1:0] true_row_val = (row_d[10] >= 3) ? row_d[10] - 3 : 0;

    wire in_col_range = (true_col_val >= 3) && (true_col_val < LINE_WIDTH - 3);
    wire in_row_range = (true_row_val >= 3) && (true_row_val < IMG_HEIGHT  - 3);
