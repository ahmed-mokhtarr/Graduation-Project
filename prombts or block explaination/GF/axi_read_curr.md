## AXI Read Current Module (`axi_read_curr`)

The **AXI Read Current** module acts as an AXI4 Master. It is responsible for reading the current frame's image data from main memory via the Smart Connect and writing it into a local FIFO. It utilizes a Finite State Machine (FSM) and internal counters to manage AXI burst transactions based on the requested layer dimensions.

---

### **Interface Definition**

The module communicates across three primary interfaces: the Address Calculation Unit (ACU), the target FIFO, and the AXI4 Smart Connect.

| Interface | Direction | Signal Name | Description |
| :--- | :--- | :--- | :--- |
| **ACU** | Input | `curr_addr` | Base address for the current frame to start reading from. |
...
| **Current FIFO** | Input | `fifo_full` | Indicates the FIFO is full; used to pause AXI data reception. |
...
The module's behavior is governed by a 4-state FSM. It utilizes internal counters to track the amount of data read versus the total data required for the `curr_layer`.

#### **1. IDLE**
* **Trigger:** Waits for the `start_read` pulse from the ACU.
* **Action:** Upon receiving the pulse, it latches the incoming configurations (`curr_addr` and `curr_layer`). It initializes the internal counters and calculates the initial AXI burst size and specific address boundaries.
...
* **Next State:** * **If more data is needed:** Recalculates the remaining data, updates the `curr_addr` for the next transaction, sets the next burst size, and loops back to `WRITE_ADDR`.
